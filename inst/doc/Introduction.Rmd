...
`defineClass` is an own implementation of `setRefClass` and can be considered experimental, although it has been reliable in my own work. The current recomendation, however, is to use the function `Class`, which does not implement a new logic of object orientation but fixes my major issue with `setRefClass`: defining methods inside lists. It is simply a wrapper which allows to define classes inside an R-expression, very much like in `defineClass`, and passes the arguments to `setRefClass`. The following example is taken from the documentation file of `setRefClass` so you can decide if this is an actual improvement:
